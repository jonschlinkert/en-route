 ...
 * with changes specific to handling file objects instead of requests and responses.
 */
...
proto.handle = function(file, done) {
  ...
  var parentParams = file.params;
  var parentPath = file.basePath || '';
  done = restore(done, file, 'basePath', 'next', 'params');

  // setup next layer
  file.next = next;

  // setup basic req values
  file.basePath = parentPath;
  ...
  function trimPrefix(layer, layerError, layerPath, path) {
    ...
    if (layerPath.length !== 0) {
      ...
      file.basePath = parentPath + (removed[removed.length - 1] === '/'
        ? removed.substring(0, removed.length - 1)
        : removed);
    }

    debug('%s %s : %s', layer.name, layerPath, file.originalPath);
...
proto.processParams = function(layer, called, file, done) {
  ...
  function param(err) {
    ...
    if (paramCalled && (paramCalled.error || paramCalled.match === paramVal)) {
      // restore value
      file.params[name] = paramCalled.value;
  ...
  function paramCallback(err) {
    ...
    // store updated value
    paramCalled.value = file.params[key.name];
