...
proto.use = function use(fn) {
  ...
  var callbacks = lazy.flatten([].slice.call(arguments, offset));
  ...
  while (len--) {
    ...
    if (typeof cb !== 'function') {
      throw new TypeError('Router#use() requires middleware function but got: ' + lazy.typeOf(cb));
    }
    // add the middleware
...
function mergeParams(params, parent) {
  ...
  var obj = lazy.extend({}, parent);

  // simple non-numeric merging
  if (!(0 in params) || !(0 in parent)) {
    return lazy.extend(obj, params);
  }
  ...
  return lazy.extend(parent, params);
}
