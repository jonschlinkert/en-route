...
class Layer {
  constructor(pattern, fn, options) {
    ...
    this.handler = file => {
      let params = this.match(file.path) || this.match(file.relative);
      if (params) {
  ...
  async handle(file) {
    const res = this.handler(file);
    if (!this.options.sync) await res;
    return file;
  }
  ...
  get regex() {
    ...
    if (this.pattern && !this._regex) {
      ...
      const keys = [];
      const regex = this._regex = toRegex(pattern, keys, this.options);
      regex.lastIndex = 0;
      regex.keys = keys;
      this._regex = regex;
      return regex;
