...
class Layer {
  ...
  /**
   * Create a regular expression from the given pattern, if it isn't already one.
   * @return {RegExp}
   * @api public
   */

  toRegex() {
    if (typeof this.options.toRegex === 'function') {
      return this.options.toRegex(this.pattern, this.keys, this.options);
    }
    return toRegex(this.pattern, this.keys, this.options);
  }
