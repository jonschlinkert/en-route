...
class Router extends Emitter {
  ...
  handler(method) {
    ...
    const handler = (pattern, ...fns) => {
      ...
      const route = new Route(pattern, fns, this.options);
...
function handle(router, method, file) {
  ...
  for (const route of router.routes.get(method)) route.handle(file);
  return file;
}

function all(router, file) {
  for (const method of router.methods) router.handle(method, file);
  return file;
}
