...
Router.prototype.dispatch = function () {
  ...
  debug('dispatching %j:', args);

  var self = this;
  var next = args.pop();
  var state = { args: args, i: 0 };
...
/**
 * Utilize the given middleware `fn` for the given `path`,
 * defaulting to `_/_`.
 *
 * **Example:**
 *
 ...
 *
 * ```js
 * router.runStage('init', function (a, b, next) {
 *   a.foo = {};
 *   b.bar = {};
 *   next();
 * });
...
Router.prototype.runStage = function (stage) {
  ...
  fns.forEach(function (fn) {
    if (typeof fn !== 'function') {
      var type = {}.toString.call(fn);
      var msg = 'Router.runStage() requires callback functions but got a ' + type;
      throw new Error(msg);
    }
