...
router.param = function(name, fn) {
  ...
  for (var i = 0; i < this._params.length; ++i) {
    var param = this._params[i](name, fn);
  ...
  function next(err) {
    ...
    self.matchLayer(layer, file, function(err, path) {
      ...
      self.processParams(layer, paramcalled, file, function(err) {
        ...
        trimPrefix(layer, layerPath, path);
...
router.matchLayer = function matchLayer(layer, file, cb) {
  try {
    cb(null, layer.match(file.path) ? file.path : undefined);
  } catch (err) {
    cb(err);
  }
};
...
function expected(type, val) {
  return `expected a ${type}, but received ${utils.typeOf(val)}`;
}
