...
router.processParams = function(layer, called, file, cb) {
  ...
  reduce(utils.arrayify(layer.keys), [], function(acc, key, next) {
    var name = key.name;
    ...
    var fns = self.params[name];
    if (!fns) {
      next(null, file);
      return;
    }

    // restore value if param previously called
    // with same value or error occurred
    ...
    reduce(fns, [], function(acc, paramFn, callback) {
      try {
